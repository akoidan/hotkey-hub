...
import { GetMonitorScaleFactorCommand } from '@/config/types/get-commands';

@Injectable()
export class GetMonitorScaleFactorHandler extends BaseGetHandler {
  canHandle(command: any): command is GetMonitorScaleFactorCommand {
    return command.get === 'getMonitorScaleFactor' && !!command.variables?.mid;
  }
